...
class Playlist extends React.Component {
  ...
  render() {
    ...
    let filteredTracks = R.filter((track)=>{
    return ( track.artist.toLowerCase().indexOf(this.state.searchValue.toLowerCase()) === 0 ||
            track.track.toLowerCase().indexOf(this.state.searchValue.toLowerCase()) === 0  );
    } ,this.props.tracks);
    let tracksToShow = R.values(R.mapObjIndexed((track,i,obj) => {
